...
const downloadEvalsUrl = 'https://d7o5we7e9g.execute-api.us-west-1.amazonaws.com/scu-evals'; // also in `manifest.json`
...
async function downloadEvals() {
    const { oauth_token: oAuthToken, access_token: accessToken } = await chrome.storage.sync.get(['oauth_token', 'access_token']);
    const response = await fetch(downloadEvalsUrl, { headers: {
    ...
    await chrome.storage.local.set({ evals: data.data, evals_expiration_date: data.data_expiration_date });
...
async function downloadEvalsIfNeeded() {
    ...
    const { evals_expiration_date: evalsExpirationDate } = await chrome.storage.local.get('evals_expiration_date');
