...
async function downloadEvalPdfAndExtractData(
  ...
  if (retryAttempts < 0) {
    console.error(`Couldn't download eval pdf ${link} after multiple attempts.`);
    return null;
  }
  let response;
  try {
  ...
  if (isBadResponse(response, responseText, link)) {
    ...
    await downloadEvalPdfAndExtractData(evalName, term, link, links, retryAttempts - 1);
  } else {
    await extractEvalDataFromPdf(Buffer.from(await response.arrayBuffer()), evalName, term);
  }
}

function isBadResponse(response, responseText, link) {
  if (!response || !response.headers) {
    console.log(`No response or response headers for eval pdf ${link}. Retrying...`);
    return true;
    ...
    console.error(`Bad auth when downloading eval at ${link}. Reauthenticating...`);
  }
  return false;
}
...
function isMultipleEvaluationsPage(responseText, evalLinks) {
  ...
  doc.querySelectorAll("a").forEach((link) => {
    if (link.textContent.includes(".pdf")) newLinks.push(EVALUATIONS_URL + link.href);
  });
