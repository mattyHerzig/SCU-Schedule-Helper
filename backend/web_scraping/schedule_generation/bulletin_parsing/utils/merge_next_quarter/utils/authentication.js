...
export async function authenticate(username, password) {
  ...
  while (needMobileApproval && loginTries < MAX_LOGIN_TRIES) {
    console.log("Waiting for Duo Security page...");
    loginTries++;
    console.log(
      "***************************ACTION REQUIRED***************************",
    );
    ...
    console.log("Waiting for other options button...");
    const otherOptionsButton = await page.waitForSelector(
      ".other-options-link",
    );
    console.log("Tapping other options button...");
    ...
    if (!needMobileApproval) {
      console.log("waiting for SCU request...");
      await page.waitForRequest((request) => request.url().includes("scu.edu") || request.url().includes("myworkday"));
    } else {
