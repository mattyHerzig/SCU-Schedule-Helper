...
export function parseInterestedSections(
  ...
  return Object.keys(interestedSections || {})
    .map((encodedCourse) => {
      ...
      const meetingPatternMatch = courseMatch[3].match(/(.*) \| (.*) \| (.*)/) || courseMatch[3].match(/(.*) \| (.*)/)
      let meetingPattern;
 ...
 */
export function parseTakenCourses(encodedCoursesTaken, includeKey = false) {
...
export function getRelevantCourseTimes(type) {
  if (type === "interested") {
    return [
